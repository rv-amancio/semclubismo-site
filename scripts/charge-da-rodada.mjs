...
function punchlineFor(entry, result, usedTemplateIds, usedPunchlines) {
  ...
  if (!result) {
    return uniquePunchline(
      punchlineOptions('sem-jogo', [
        `${name} nem jogou e ainda conseguiu deixar a torcida fazendo conta com medo.`,
        `${name} ficou no sofá, mas a tabela cutucou como se tivesse entrado em campo.`,
        `${name} descansou a chuteira; a cobrança, infelizmente para eles, não descansou.`,
        `${name} folgou no campo e trabalhou dobrado no tribunal da resenha.`,
        `${name} viu a rodada passar com aquela paz falsa de quem sabe que deve futebol.`,
        `${name} não entrou em campo; a ansiedade entrou e correu mais que o time.`,
        `${name} ficou parado torcendo para ninguém expor o tamanho do problema.`,
        `${name} virou secador profissional, porque jogar bola fica para a próxima.`,
        `${name} teve rodada de observação e saiu observado pela própria torcida.`,
        `${name} não jogou; mesmo assim a corneta achou expediente.`,
        `${name} ficou sem placar próprio e com medo do placar dos outros.`,
        `${name} tirou a noite de campo, mas a tabela apareceu cobrando aluguel.`,
        `${name} virou fiscal da rodada enquanto tenta parecer no controle.`,
        `${name} não suou camisa, só paciência de quem acompanha.`,
        `${name} ficou no modo espera: menos gramado, mais crise imaginada.`,
        `${name} assistiu ao campeonato pela janela e fingiu que era estratégia.`,
        `${name} descansou, mas a torcida seguiu em plantão de reclamação.`,
        `${name} ficou sem jogo e ainda assim quase arrumou pauta negativa.`,
        `${name} poupou as chuteiras e gastou todos os álibis possíveis.`,
        `${name} não teve 90 minutos; teve 90 cenários ruins no grupo da torcida.`,
        `${name} assistiu ao caos dos outros rezando para não virar o próximo capítulo.`,
      ]),
      seed,
      usedTemplateIds,
      usedPunchlines,
  ...
  if (result.outcome === 'win' && result.diff >= 3) {
    return uniquePunchline(
      punchlineOptions('win-goleada', [
        `${name} goleou e a soberba já entrou em campo de terno.`,
        `${name} passou o rodo e deixou ${opponent} procurando recibo da vergonha esportiva.`,
        `${name} venceu largo; até quem corneta teve que assinar presença na festa.`,
        `${name} fez placar de manchete e já tem torcedor falsificando dinastia.`,
        `${name} ganhou com folga e agora precisa fingir humildade por 48 horas.`,
        `${name} abriu vantagem no placar e fechou a porta da desculpa adversária.`,
        `${name} atropelou o jogo e deixou a resenha com laudo, foto e testemunha.`,
        `${name} resolveu cedo e passou o resto do jogo humilhando a ansiedade alheia.`,
        `${name} venceu tão largo que a prudência da torcida foi dispensada.`,
        `${name} aplicou corretivo esportivo e ainda pediu close no replay.`,
        `${name} ganhou grande; controlar a empolgação virou trabalho de segurança.`,
        `${name} fez a diferença de gols virar arma pesada no almoço de família.`,
      ]),
      seed,
      usedTemplateIds,
      usedPunchlines,
  ...
  if (result.outcome === 'win' && result.homeAway === 'away') {
    return uniquePunchline(
      punchlineOptions('win-fora', [
        `${name} venceu fora e voltou com três pontos e um discurso insuportável.`,
        `${name} ganhou longe da torcida e já vende maturidade como se fosse troféu.`,
        `${name} buscou vitória fora; a arrogância deve chegar antes do ônibus.`,
        `${name} calou a casa alheia e deixou recibo colado na testa da tabela.`,
        `${name} foi visitante só no ingresso; no placar entrou mandando fechar a conta.`,
        `${name} venceu contra ${opponent} e trouxe deboche na bagagem de mão.`,
        `${name} voltou da viagem com três pontos e alergia à modéstia.`,
        `${name} ganhou fora, esse luxo que faz torcedor esquecer os pecados recentes.`,
        `${name} entrou na casa dos outros e saiu levando a chave da resenha.`,
        `${name} achou vitória longe de casa e já tem gente gritando arrancada sem pudor.`,
        `${name} venceu fora; a tabela sorriu e o rival engoliu seco.`,
        `${name} buscou três pontos na estrada e voltou com pose de quem descobriu futebol.`,
      ]),
      seed,
      usedTemplateIds,
      usedPunchlines,
  ...
  if (result.outcome === 'win') {
    return uniquePunchline(
      punchlineOptions('win-normal', [
        `${name} fez o mínimo e embalou como obra-prima. Futebol também é marketing.`,
        `${name} ganhou, respirou e empurrou a crise para baixo do tapete.`,
        `${name} somou três pontos e liberou a torcida para ficar chata até a próxima.`,
        `${name} venceu e autorizou exagero moderado, o que ninguém vai respeitar.`,
        `${name} achou a vitória e já tem gente fingindo que sempre acreditou.`,
        `${name} fez o básico; no Brasileirão, isso já parece milagre administrativo.`,
        `${name} ganhou e vai passar 24 horas jurando que estava tudo planejado.`,
        `${name} saiu com vitória e munição para calar parente inconveniente.`,
        `${name} venceu; resta saber se foi reação ou só o adversário colaborou.`,
        `${name} colocou três pontos no bolso e mandou a cobrança esperar no corredor.`,
        `${name} ganhou em casa e transformou incêndio em fumacinha controlada.`,
        `${name} venceu contra ${opponent}; a corneta não morreu, só perdeu o sinal.`,
      ]),
      seed,
      usedTemplateIds,
      usedPunchlines,
  ...
  if (result.outcome === 'draw' && result.goalsFor === 0) {
    return uniquePunchline(
      punchlineOptions('draw-zero', [
        `${name} entregou um 0 x 0 que deveria vir com pedido de desculpas ao público.`,
        `${name} empatou sem gols; o placar trabalhou mais que o ataque.`,
        `${name} saiu com um ponto e quase nada para defender sem gaguejar.`,
        `${name} fez do 0 x 0 uma punição coletiva com transmissão oficial.`,
        `${name} colocou o torcedor para ver a esperança evaporar em alta definição.`,
        `${name} empatou no zero e deixou o melhor lance na imaginação de quem sofreu.`,
        `${name} saiu de um 0 x 0 que deveria render adicional de paciência premium.`,
        `${name} dividiu pontos e escondeu os gols em algum lugar que ninguém achou.`,
        `${name} ficou no zero; a rede pediu férias e foi atendida.`,
        `${name} empatou sem gol, porque finalizar aparentemente era opcional.`,
        `${name} transformou o 0 x 0 em teste de resistência para gente teimosa.`,
        `${name} levou um ponto e deixou a empolgação trancada no porta-malas.`,
        `${name} fez um empate tão seco que até a súmula pediu água.`,
        `${name} saiu sem balançar a rede e cheio de frase pronta para vender solidez.`,
        `${name} segurou o zero dos dois lados e chamou de estratégia para não chorar.`,
        `${name} empatou com ${opponent} e provou que o nada também faz barulho.`,
        `${name} passou 90 minutos ameaçando jogar futebol e desistiu no detalhe.`,
        `${name} fez a bola circular, mas proibiu o placar de participar.`,
        `${name} buscou o gol como quem procura dignidade em coletiva pós-empate.`,
        `${name} fechou o jogo no zero e abriu falência temporária da paciência.`,
        `${name} terminou sem gols; ninguém acusou o ataque de excesso de serviço.`,
      ]),
      seed,
      usedTemplateIds,
      usedPunchlines,
  ...
  if (result.outcome === 'draw') {
    return uniquePunchline(
      punchlineOptions('draw-normal', [
        `${name} empatou e chamou de ponto importante. A tabela fingiu respeito.`,
        `${name} dividiu pontos, culpa e desculpas em parcelas vergonhosamente iguais.`,
        `${name} ficou no meio do caminho: nem crise assumida, nem alegria decente.`,
        `${name} levou um ponto para casa e abandonou dois no balcão das explicações.`,
        `${name} empatou, aquele resultado que se defende olhando para o rodapé.`,
        `${name} saiu com um ponto e uma mentira piedosa para acalmar a torcida.`,
        `${name} ficou no quase contra ${opponent}: quase venceu, quase enganou.`,
        `${name} somou um ponto e multiplicou o estoque de desculpas.`,
        `${name} empatou e deixou a pergunta incômoda: evolução ou desperdício gourmet?`,
        `${name} saiu igual no placar e em dívida com a paciência da torcida.`,
        `${name} dividiu a conta com ${opponent}, mas pagou sozinho o vexame moral.`,
        `${name} não perdeu, também não resolveu. A classificação anotou a covardia.`,
        `${name} empatou e entregou aquele recibo de oportunidade jogada fora.`,
        `${name} fez jogo de meio termo: serve para coletiva, não serve para convencer.`,
        `${name} ficou no empate e deixou a rodada com gosto de pipoca fria.`,
        `${name} ganhou um ponto e perdeu a chance de parar a choradeira.`,
        `${name} assinou empate e mandou a torcida procurar consolo no replay.`,
        `${name} trouxe um ponto, mas esqueceu a vergonha no vestiário adversário.`,
        `${name} saiu do empate com cara de quem sabe que desperdiçou a noite.`,
        `${name} repartiu o placar e deixou a cobrança em condomínio de luxo.`,
        `${name} empatou com ${opponent}; a calculadora sofreu mais que a torcida.`,
      ]),
      seed,
      usedTemplateIds,
      usedPunchlines,
  ...
  if (result.outcome === 'loss' && result.diff <= -3) {
    return uniquePunchline(
      punchlineOptions('loss-pancada', [
        `${name} tomou pancada e a reunião de cobrança já nasceu gritando.`,
        `${name} perdeu feio: quando o placar vira prova, coletiva bonita vira enfeite.`,
        `${name} levou uma dessas que fazem até o replay ter vergonha de voltar.`,
        `${name} virou alerta em caixa alta antes do vestiário fechar a porta.`,
        `${name} saiu amassado no placar e sem crédito na paciência da torcida.`,
        `${name} levou goleada e transformou a coletiva em interrogatório esportivo.`,
        `${name} perdeu grande; nem o otimismo passou pela catraca.`,
        `${name} tomou distância no placar e encostou de vez na cobrança.`,
        `${name} apanhou da partida e deixou a tabela com cara de bronca pública.`,
        `${name} entregou um placar que não cabe em desculpa curta nem longa.`,
        `${name} saiu de campo com derrota que vira pauta, meme e reunião emergencial.`,
        `${name} sofreu uma dessas que fazem a semana começar pedindo explicação.`,
      ]),
      seed,
      usedTemplateIds,
      usedPunchlines,
  ...
  if (result.outcome === 'loss' && result.homeAway === 'home') {
    return uniquePunchline(
      punchlineOptions('loss-casa', [
        `${name} perdeu em casa e transformou apoio em silêncio de estádio vazio.`,
        `${name} tropeçou diante da própria torcida. O visitante agradeceu o mimo.`,
        `${name} deixou os pontos em casa, mas na mala de ${opponent}.`,
        `${name} mandou no endereço; no futebol, quem mandou foi o rival.`,
        `${name} perdeu em casa; o fator local pediu para ser removido do release.`,
        `${name} abriu a porta para ${opponent} e ainda ofereceu três pontos no capricho.`,
        `${name} fez a torcida sair revisando promessa, escalação e paciência esportiva.`,
        `${name} caiu diante da própria arquibancada e ouviu até silêncio reclamar.`,
        `${name} perdeu onde precisava mandar e vai vender isso como aprendizado caro.`,
        `${name} transformou mando de campo em passeio turístico para o adversário.`,
        `${name} deixou a casa arrumada para ${opponent} fazer festa sem cerimônia.`,
  ...
  return uniquePunchline(
    punchlineOptions('loss-normal', [
      `${name} voltou sem pontos e com uma mala cheia de constrangimento esportivo.`,
      `${name} perdeu e vai chamar cobrança de processo, porque coragem é opcional.`,
      `${name} caiu no placar e entregou munição premium para a resenha alheia.`,
      `${name} saiu derrotado e levou para casa aquele silêncio que humilha sem palavrão.`,
      `${name} deixou o jogo com menos pontos e mais desculpas que finalização.`,
      `${name} perdeu contra ${opponent}; a tabela não aceita drama criativo.`,
      `${name} voltou vazio no placar e lotado de frase defensiva.`,
      `${name} perdeu e fez a calculadora da torcida pedir afastamento.`,
      `${name} caiu fora de casa e trouxe a cobrança na poltrona da frente.`,
      `${name} saiu procurando onde deixou a reação que prometeu na semana passada.`,
      `${name} perdeu; a classificação não gritou, só abriu um sorriso cruel.`,
      `${name} deixou três pontos escaparem e ganhou uma semana de julgamento público.`,
    ]),
    seed,
    usedTemplateIds,
    usedPunchlines,
